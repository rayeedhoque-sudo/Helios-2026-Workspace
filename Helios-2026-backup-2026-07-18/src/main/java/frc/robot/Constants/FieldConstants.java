...
public class FieldConstants {
    ...
    //HELPERS (pure lookups -- no I/O, no state)
        ...
        // A botpose solve that lands outside the field is garbage -- shared reject for the
        // drivetrain's vision seeding AND the shooter's distance source (one rule, one place).
        public static boolean isInsideField(Pose2d pose) {
            return pose.getX() >= 0 && pose.getX() <= FIELD_LENGTH_METERS
