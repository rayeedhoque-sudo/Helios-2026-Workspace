...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
    ...
    private void updateVisionPose() {
        ...
        if (mt1 != null && mt1.tagCount >= 1
                ...
                && FieldConstants.isInsideField(mt1.pose)) {
            ...
            if (DriverStation.isDisabled()) {
                // Single-tag PnP at an oblique angle flips between the true and mirrored
                // solution frame-to-frame (the classic AprilTag ambiguity) -- adopting a
                // flipped frame pre-match would start auto from a ~180-deg-wrong heading
                // that nothing in-match can repair (MT2 never corrects yaw). Require a
                // multi-tag solve or a decisively unambiguous single tag.
                boolean unambiguous = mt1.tagCount >= 2
                    || (mt1.rawFiducials != null && mt1.rawFiducials.length > 0
                        && mt1.rawFiducials[0].ambiguity < 0.3);
                if (unambiguous && (posErrMeters > Vision.VISION_SEED_POS_TOL_METERS
                        || yawErrDeg > Vision.VISION_SEED_YAW_TOL_DEG)) {
                    this.resetPose(mt1.pose);
                    lastVisionFixSec = Timer.getFPGATimestamp();
        ...
        if (!FieldConstants.isInsideField(mt2.pose)) {
            visionTagCountPub.set(0); // garbage solve (usually a wrong camera mount pose)
            return;
        }
        if (mt2.avgTagDist > ShooterSubsystemConstants.SHOT_MAX_TAG_DIST_METERS) {
            // One trust standard: the shooter refuses raw solves past this distance, so a
            // far solve must not launder itself into the shot through the fused pose (it
            // would stamp hasRecentVisionFix and pull the estimator toward the same ~d^2
            // noise the gate exists to reject). Odometry alone carries the pose out here.
    ...
    // FPGA time of the last ACCEPTED vision correction (seed or MT2 fusion).
    private double lastVisionFixSec = Double.NEGATIVE_INFINITY;

    /** True while the pose estimator ingested an accepted vision fix within the last
     *  {@code maxAgeSec} -- i.e. the FUSED pose is fresher than dead-reckoned odometry.
     *  The shooter uses this to trust the fused pose for distance when the raw botpose
     *  fails its quality gates. */
    public boolean hasRecentVisionFix(double maxAgeSec) {
        return Timer.getFPGATimestamp() - lastVisionFixSec <= maxAgeSec;
    }
