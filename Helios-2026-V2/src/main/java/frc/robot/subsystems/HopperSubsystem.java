...
import frc.robot.constants.Constants.HopperSubsystemConstants;
...
public class HopperSubsystem extends SubsystemBase{
    ...
    public HopperSubsystem(){
        // Conservative-start current limits + neutral mode (Hardware-Data-Sheet sec.7). TODO tune on robot.
            ...
            //
            // 20 A smart limit / Coast / kResetSafeParameters via the shared factory; the belt-
            // specific disableFollowerMode + inverted(false) are applied through its extra
            // customizer (same config, same order as before -- just centralized).
            REVLibError hopperACfg = MotorConfigs.configureSparkMax(hopperMotorA, 20, IdleMode.kCoast, 0.25,
                ResetMode.kResetSafeParameters, c -> { c.disableFollowerMode(); c.inverted(false); });
            REVLibError hopperBCfg = MotorConfigs.configureSparkMax(hopperMotorB, 20, IdleMode.kCoast, 0.25,
                ResetMode.kResetSafeParameters, c -> { c.disableFollowerMode(); c.inverted(false); });
     ...
     * belts run at HOPPER_SPEED while beltsOn, kicker runs at INDEXER_SPEED while kickerOn.
     * A gate going false stops that motor the same loop.
     *
     * <p>The gates are whatever the CALLER passes. As of 2026-07-18 all three call sites
     * (RobotContainer RT, RB and test-mode Y) pass {@code () -> true} for the belts and
     * {@code shooterSS::isFlywheelAtSpeed} for the kicker -- so the kicker gate is SPEED ONLY.
     * This javadoc used to claim it was {@code isReadyToShoot()} (speed AND hood angle AND aim);
     * it is not, and has not been since the hood encoder work. The kicker will fire with the
     * hood wherever it happens to be, including in the pit under test-mode Y. Tighten the call
     * sites to {@code isReadyToShoot} once the hood feedback is trusted -- do not "fix" it here,
     * this method is correct as a mechanism.
     */
    ...
    // LIMIT on a tier change -- config only, never a control request or a setpoint). The governor
    // replays this motor's FULL recipe via MotorConfigs.rescaler, so the disableFollowerMode +
    // inverted above are re-stated every time and can never be dropped by a partial config.
