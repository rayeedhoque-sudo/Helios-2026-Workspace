...
import frc.robot.constants.Constants.ShooterSubsystemConstants;
...
public class ShooterSubsystem extends SubsystemBase{
    ...
    //Data
        private final ShuffleboardTab ShooterSubsystemTab = Shuffleboard.getTab("Shooter Subsystem Tab");
            ...
            // True while the hood stall guard is refusing to keep driving into a jam. Watch this
            // on the robot: it must stay FALSE through a full MIN->MAX->MIN hood sweep.
            private final GenericEntry hoodStalledEntry;
    ...
    // now gates its kicker through the isReadyToShoot() supplier instead.
       ...
       // Debounced copy of velReached that feeds ONLY the kicker gate (isFlywheelAtSpeed). When
       // AT_SPEED_DEBOUNCE_SEC > 0 (currently 0.06 s) a rising-edge debounce filters the gate so a
       // single noisy sample can't open the kicker early; at 0 the raw signal is used directly
       // (byte-identical to V1). Release is immediate either way -- see the constant.
       private boolean flywheelAtSpeed = false;
       private final Debouncer atSpeedDebouncer =
           new Debouncer(ShooterSubsystemConstants.AT_SPEED_DEBOUNCE_SEC, Debouncer.DebounceType.kRising);
       ...
       // BROWNOUT BUDGET (team decision 2026-07-24): during RT the drivetrain ROTATES to the
       // firing bearing while this subsystem would otherwise already be spinning four flywheel
       // Krakens up from zero -- drive burst + steer slew + flywheel spin-up stacked in one
       // window, well past the current budget TunerConstants is sized for.
       //
       // So the model velocity is withheld until the aim has actually been made once. LATCHED,
       // not live: once the aim gate passes during a hold the wheels stay commanded, otherwise a
       // small drift back off-target would drop them to zero and the shot would oscillate. Reset
       // at the start and end of every RT press. The HOOD is never withheld -- it stages during
       // the aim (it is one small NEO 550, not part of the problem) so it is already in position.
       //
       // COST: spin-up time is now added AFTER aiming instead of overlapping it, so the first
       // shot of a press is slower. If the fire rate is unacceptable on the robot, the cheaper
       // half of this mitigation is the rotation-rate cap
       // (FieldConstants.AIM_MAX_ROTATION_RATE_RAD_PER_SEC) -- keep that and revert this latch.
       private boolean aimAchieved = false;
       // Hood stall guard state (see the HOOD_STALL_* block in ShooterSubsystemConstants).
       // hoodStallDirection is 0 = free, +1 = jammed while raising, -1 = jammed while lowering.
       private final Debouncer hoodStallDebouncer =
           new Debouncer(ShooterSubsystemConstants.HOOD_STALL_DEBOUNCE_SEC, Debouncer.DebounceType.kRising);
       private double hoodStallDirection = 0;
       ...
       public ShooterSubsystem(CommandSwerveDrivetrain drivetrain){
            ...
            //Coniguring Motors
                ...
                // Same 80 A stator / 25 A supply / Coast applied to all 4 via the shared factory
                // (MotorConfigs.configureTalonFX), which enforces limits+neutral at the call site.
                // A failed apply (CAN timeout = controller unpowered or off the bus) leaves that
                // Kraken with NO current limit and the vendor-default Coast -- four unlimited
                // flywheels is precisely this team's brownout history. Report it LOUDLY rather
                // than discovering it as a brownout, matching HopperSubsystem's pattern.
                for (TalonFX flywheel : new TalonFX[] { shooterA, shooterB, shooterC, shooterD }) {
                    var flywheelCfg = MotorConfigs.configureTalonFX(flywheel, 80, 25, NeutralModeValue.Coast, 0);
                    if (!flywheelCfg.isOK()) {
                        DriverStation.reportError("Shooter flywheel (CAN " + flywheel.getDeviceID()
                            + ") configure FAILED: " + flywheelCfg
                ...
                // A failed configure here is worse than it looks: the NEO 550 comes up unlimited
                // (it stalls at 100 A and has almost no thermal margin) AND in the vendor-default
                // COAST, which turns the documented "0 V, brake idle holds the hood" safe state
                // into the hood dropping under gravity.
                REVLibError hoodCfg = MotorConfigs.configureSparkMax(shooterAngle, 20, IdleMode.kBrake, 0,
                    ResetMode.kNoResetSafeParameters, null);
                if (hoodCfg != REVLibError.kOk) {
                    DriverStation.reportError("Hood (CAN " + ShooterSubsystemConstants.SHOOTER_ANGLE_ID
                        + ") configure FAILED: " + hoodCfg
                        + " -- NEO 550 may be unlimited and in COAST (hood can drop). Check CAN/power.", false);
                }
            ...
            //Initializing Shuffleboard Entries
                ...
                hoodStalledEntry = ShooterSubsystemTab.add("Hood Stall Cutoff", false).getEntry();
       }
    ...
    //Utility Methods
        ...
        /**
         * Hood angle (deg) from a raw absolute-encoder sample the caller ALREADY read. Takes the
         * sample as an argument on purpose: periodic() used to call a no-argument version that
         * re-read the encoder each time, so the feedback-sanity gate validated one sample while
         * the PID closed the loop on a different, later one -- a glitch frame landing between the
         * two reads would pass the gate and then be fed straight to the hood. One sample per loop
         * now feeds the gate, the PID, the at-angle gate and the dashboard alike.
         */
        private static double getShooterAngleDegrees(double raw){
    ...
    //Subsystem Methods
        ...
        // Raw flywheel at-speed gate: within tolerance of a NONZERO commanded surface speed.
        // Pure + package-private so ShotModelTest can pin it. A zero command can never read
        // at-speed, which keeps the kicker off stopped wheels.
        static boolean atSpeed(double desiredSurfaceSpeed, double measuredSurfaceSpeed,
                               double toleranceSurfaceSpeed){
            return desiredSurfaceSpeed != 0
                && Math.abs(desiredSurfaceSpeed - measuredSurfaceSpeed) < toleranceSurfaceSpeed;
        }
        ...
        private void runVisionTargeting(){
            ...
            // Field-pose source: ONE pose for everything -- the drivetrain's FUSED estimate
            // (MegaTag2-corrected odometry), used for the distance, the aim bearing AND the
            // isAimedAtTarget() gate.
            //
            // It used to take the distance and bearing from the RAW MegaTag1 botpose while the
            // aim gate compared the robot's heading against the FUSED pose. Those are two
            // different frames: whenever they disagreed (they can differ by degrees on a
            // single-tag solve) the servo chased a bearing computed in one frame and was judged
            // "aimed" in another, so it could latch while pointing off-target, or never latch at
            // all. Reading one pose removes that class of error, and the fused pose is also
            // smoother -- odometry fills in between camera frames, so the aim stops jittering at
            // the camera's frame rate.
            //
            // Guard: the fused pose is only trustworthy for RANGE once vision has actually
            // corrected it (isVisionFresh). Without that it is dead reckoning from wherever
            // odometry was zeroed, which would yield a confident, wrong distance. When it is not
            // fresh we fall through to the camera-space fallback below, exactly as before.
            Pose2d fieldPose = drivetrain.isVisionFresh() ? drivetrain.getState().Pose : null;
            ...
            if (fieldPose != null) {
                ...
                // FEED with no vision-backed fused pose: a camera-space tag cannot locate a field
                // aim point (the feed target is a field coordinate, not a tag), so REFUSE (no
                // shooting operations). The driver
            ...
            // Latch the aim once it is first made this press (see aimAchieved). hasShotTarget
            // still reflects the MODEL result, not the gated command, so the dashboard shows a
            // valid firing solution while the robot is still turning onto it.
            hasShotTarget = vSurface > 0;
            if (!aimAchieved && aimHeadingValid && isAimedAtTarget()) {
                aimAchieved = true;
            }
            setDesiredFlywheelVelocity(aimAchieved ? vSurface : 0);
        }
        ...
        public Command visionShotCommand(){
            return runEnd(
                ...
                () -> {
                    ...
                    aimAchieved = false;          // next press must aim again before spinning up
                })
    ...
    public void periodic(){
        // ONE hood encoder sample for the whole loop. The feedback-sanity gate, the PID, the
        // at-angle gate and every dashboard readout below all judge THIS reading, so they can
        // never disagree with each other about where the hood is.
        final double rawHood = shooterAngleEncoder.getPosition();
        final double currentHoodAngle = getShooterAngleDegrees(rawHood);
        ...
        if(enableSubsystem){
            ...
            //Shooter Angle
                ...
                // STALL CUTOFF (see the HOOD_STALL_* block in ShooterSubsystemConstants). Measured
                // current + a stopped encoder, debounced -- never commanded output. Latches the
                // DIRECTION that jammed and refuses only that direction, so the hood can always be
                // driven back off the jam; a plain cutoff would chatter (cut -> current drops ->
                // un-cut -> push again) and still cook the 550 at high duty.
                boolean hoodJammed = Math.abs(hoodVolts) > ShooterSubsystemConstants.HOOD_STALL_MIN_VOLTS
                    && shooterAngle.getOutputCurrent() >= ShooterSubsystemConstants.HOOD_STALL_CURRENT_AMPS
                    && Math.abs(shooterAngle.getEncoder().getVelocity()) < ShooterSubsystemConstants.HOOD_STALL_MOTOR_RPM;
                if (hoodStallDebouncer.calculate(hoodJammed)) {
                    hoodStallDirection = Math.signum(hoodVolts);
                }
                if (hoodStallDirection != 0) {
                    if (hoodVolts != 0 && Math.signum(hoodVolts) != hoodStallDirection) {
                        hoodStallDirection = 0;   // now driving AWAY from the jam -- allow it
                    } else {
                        hoodVolts = 0;            // still pushing into the jam -- refuse; brake idle holds
                    }
                }
            ...
            // ShooterSubsystemConstants are DELETED; the hopper polls isReadyToShoot()).
                // Raw at-speed (unchanged semantics): feeds telemetry + isReadyToShoot().
                velReached = atSpeed(desired_Velocity, getShooterFlywheelVelocity(),
                    ShooterSubsystemConstants.SPEED_TOLERANCE);
                // Debounced copy feeds ONLY the kicker gate. Fed EVERY loop HERE (not in the
                // getter, which is polled only while the feed command runs). debounce > 0 (0.06 s)
                // -> rising-edge filter on the gate; debounce 0 -> raw signal (byte-identical to V1).
                flywheelAtSpeed = ShooterSubsystemConstants.AT_SPEED_DEBOUNCE_SEC > 0
                    ? atSpeedDebouncer.calculate(velReached)
                    : velReached;
                angleReached =
                     desired_Angle != 0 ? Math.abs(desired_Angle-currentHoodAngle) < ShooterSubsystemConstants.ANGLE_TOLERANCE : false;
            //Data
                // The six PID/FF entries are dashboard INPUTS, not outputs. They used to be
                // written from the live gains right here, immediately before the "PID + FF
                // Tuning" block below read them back and compared -- so the read always returned
                // exactly what had just been written, the comparison was never unequal, and a
                // slider edit was stomped a loop later without ever being applied. Live PID
                // tuning (a durable team pattern) was silently dead for both the speed and angle
                // loops. The constructor already seeds all six from the boot gains, so the
                // widgets still show correct values; now an edit actually survives to be applied.
            ...
            //PID + FF Tuning
                ...
                velReached = false;
                angleReached = false;
                // Reset the kicker at-speed debounce too, so a value latched true before a
                // mid-shot disable can't re-open the kicker on re-enable.
                flywheelAtSpeed = false;
                atSpeedDebouncer.calculate(false);
                hasShotTarget = false;
                aimHeadingValid = false;
                // Clear the hood stall guard: the hood is not being driven while disabled, so a
                // latch from before must not survive into the next enable.
                hoodStallDirection = 0;
                hoodStallDebouncer.calculate(false);
            }
        ...
        // "Desired Velocity Reached" at their boot values while disabled).
            ...
            currentAngleEntry.setDouble(currentHoodAngle);
            ...
            rawHoodEncoderEntry.setDouble(rawHood);
            hoodStalledEntry.setBoolean(hoodStallDirection != 0);
    ...
    // Motor access for PowerTelemetry (reads only) and BatteryGovernor (re-applies the SUPPLY
    // current limit on a tier change -- config only, never a control request or a setpoint).
    public TalonFX[] getFlywheelMotors(){
        return new TalonFX[] { shooterA, shooterB, shooterC, shooterD };
    }

    // Motor access for PowerTelemetry (reads only) and BatteryGovernor (config only, never a
    // control request). The hood is registered shot-critical AND floored at its full 20 A limit,
    // so the governor can never actually reduce it -- see the table in RobotContainer.
