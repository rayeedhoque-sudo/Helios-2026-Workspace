...
public class FieldConstants {
    ...
    //DRIVETRAIN-FACING TUNABLES (owned here so the shooter/drivetrain agents share one source)
        ...
        // Rotation-rate cap for the RT auto-aim servo ONLY (drivetrain.aimUntilAligned), rad/s.
        // BROWNOUT BUDGET (team decision 2026-07-24): RT is the one binding that rotates the robot
        // while the flywheels are also being commanded, so it stacks drive + steer + flywheel draw
        // in one window. The other heading servos (rotateToAngle, searchAndAlignCommand) never run
        // with the flywheels and keep the original 1.5*pi.
        // Halving the commanded rate quarters the rotational kinetic energy the drives have to put
        // in, which is where the current burst comes from -- the P-only servo commands its cap
        // immediately on any large error. pi rad/s = 0.5 rot/s is still faster than the 0.8 rot/s
        // teleop spin feels, because this is an in-place rotation with no translation.
        // TODO tune on robot: raise toward 1.5*pi if the aim feels sluggish, but watch the
        // CompanionTelemetry battery voltage trace during an RT press on a half-drained pack.
        public static double AIM_MAX_ROTATION_RATE_RAD_PER_SEC = Math.PI;
