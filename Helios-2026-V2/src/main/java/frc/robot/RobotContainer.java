...
import frc.robot.subsystems.MatchStatus;
import frc.robot.subsystems.ShooterSubsystem;
import frc.robot.util.BatteryGovernor;
import frc.robot.util.LimelightThermalManager;
...
public class RobotContainer {
    ...
    // Limelight LED-off + shooter-idle low-res pipeline + over-temp warning. Auto-runs via the
    // scheduler. "Shooter active" = the camera is needed at full resolution, which is EITHER the
    // flywheels being commanded to spin OR a shot currently being aimed.
    //
    // The hasShotTarget() term matters since 2026-07-24: RT now withholds flywheel spin-up until
    // the aim is made (see ShooterSubsystem.aimAchieved), so velocity is 0 for the whole aim. On
    // velocity alone the camera would read as IDLE during exactly the window the aim depends on
    // it -- and if idle-pipeline switching is ever re-enabled (LL_PIPELINE_IDLE is currently ==
    // NORMAL, which disables it) that would blind auto-aim while it is trying to acquire.
    public final LimelightThermalManager limelightThermal =
        new LimelightThermalManager(Vision.CAM_LIMELIGHT,
            () -> shooterSS.getDesiredFlywheelVelocity() != 0 || shooterSS.hasShotTarget());

    // Battery-voltage-adaptive current limits across every mechanism subsystem. SHIPS DISABLED
    // (BatteryGovernor.ENABLED) -- it runs the ladder and publishes the tier it WOULD have shed to,
    // and writes to no motor, until the brownout cause is actually measured. Swerve is excluded
    // (Tuner X owns those limits; scale MaxSpeed instead) and the kicker Victor SPX cannot be
    // governed at all. Every floor below is [assumed] -- TODO confirm each on the robot.
    //
    //   motor                base   floor  sheds at   shot-crit  why that floor
    //   flywheel x4 (FX)     25 A   15 A   TIER_SHOT  yes   above steady windage draw; a shed may
    //                                                       only slow spin-up, never bog a shot
    //   intake roller (FX)   25 A   14 A   TIER_FEED  no    worst-case steady = outtake 0.3 duty x
    //                                                       40 A stator = 12 A (Constants) + margin
    //   hopper belt x2 (SM)  20 A   10 A   TIER_FEED  no    belts free-run ~0.1 A measured
    //                                                       2026-07-18; the 20 A is a stall cap
    //   intake slider (SM)   15 A   15 A   TIER_FEED  no    FLOOR = BASE, cannot shed: the stall
    //                                                       cutoff only fires because current can
    //                                                       peg at SLIDER_STALL_CURRENT_AMPS = 12
    //   hood (SM)            20 A   20 A   TIER_FEED  yes   FLOOR = BASE, cannot shed: same
    //                                                       coupling (HOOD_STALL_CURRENT_AMPS = 15)
    //                                                       and hood angle IS shot accuracy
    //
    // The shot interlock reuses the same "the shooter is working" predicate as the Limelight
    // manager above: flywheels commanded to spin, OR a shot currently being aimed.
    public final BatteryGovernor batteryGovernor = new BatteryGovernor()
        .protectShotWhile(() -> shooterSS.getDesiredFlywheelVelocity() != 0 || shooterSS.hasShotTarget())
        .register(15.0, BatteryGovernor.TIER_SHOT, true,  shooterSS.getFlywheelMotors())
        .register(14.0, BatteryGovernor.TIER_FEED, false, intakeSS.getRollerMotor())
        .register(10.0, BatteryGovernor.TIER_FEED, false, hopperSS.getHopperMotors())
        .register(15.0, BatteryGovernor.TIER_FEED, false, intakeSS.getSliderMotor())
        .register(20.0, BatteryGovernor.TIER_FEED, true,  shooterSS.getHoodMotor());
        ...
        //DRIVE SUBSYSTEM
            ...
            // the two commands is negligible for the slew clock).
                ...
                drivetrain.setDefaultCommand(
                    ...
                    // MODE GATE (safety, team decision 2026-07-24): the sticks only drive the
                    // robot in TELEOP. A default command has no mode gate of its own, so without
                    // this the robot was fully driveable from the sticks in TEST mode -- which is
                    // exactly when people have hands inside the intake and hopper running the
                    // per-mechanism test binds -- and in AUTONOMOUS whenever no auto command was
                    // holding the drivetrain. Costs nothing: no test binding uses the drivetrain,
                    // and DISABLED already has its own Idle binding below.
