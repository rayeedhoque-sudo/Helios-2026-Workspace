...
public class PowerTelemetry {
    ...
    // CAN bus health. This robot puts 18 motor controllers, 4 CANcoders and a Pigeon 2 on ONE
    // roboRIO CAN 2.0 bus (1 Mbit/s), and nothing was measuring how full it is. A saturated bus
    // delays the swerve's own position/velocity frames, which degrades odometry -- i.e. it shows
    // up as a bad pose, not as an obvious "CAN error". Watch BusUtilization during a full-load
    // practice match: below ~70% there is nothing to do, and above it the fix is to slow specific
    // status frames. Measure before optimizing -- blindly slowing frames can starve the intake
    // slider's current/velocity stall detection, which reads at loop rate.
    // REC/TEC are the CAN receive/transmit error counters (non-zero = wiring/termination trouble)
    // and TxFullCount counts dropped transmits.
    private final CANBus canBus = TunerConstants.kCANBus;
    private final DoublePublisher busUtilPub;
    private final DoublePublisher canRecPub;
    private final DoublePublisher canTecPub;
    private final DoublePublisher canTxFullPub;
    private final DoublePublisher canOffCountPub;
    ...
    public PowerTelemetry(CommandSwerveDrivetrain drivetrain, ShooterSubsystem shooterSS,
        ...
        busUtilPub = table.getDoubleTopic("canBusUtilization").publish();
        canRecPub = table.getDoubleTopic("canRxErrors").publish();
        canTecPub = table.getDoubleTopic("canTxErrors").publish();
        canTxFullPub = table.getDoubleTopic("canTxFullCount").publish();
        canOffCountPub = table.getDoubleTopic("canBusOffCount").publish();
    ...
    private void update() {
        ...
        // CAN bus health (see canBus field). Read here, off the main loop, at 10 Hz.
        var canStatus = canBus.getStatus();
        busUtilPub.set(canStatus.BusUtilization);
        canRecPub.set(canStatus.REC);
        canTecPub.set(canStatus.TEC);
        canTxFullPub.set(canStatus.TxFullCount);
        canOffCountPub.set(canStatus.BusOffCount);
    }
}
